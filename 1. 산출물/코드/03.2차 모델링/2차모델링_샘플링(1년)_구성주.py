...
cur.execute(
    f"""
    drop table if exists 타겟마트3_sample
    """
)
...
cur.execute(
    f"""
    create table if not exists 타겟마트3_sample(
      기준년월
    , 고객ID
    , 장르
    , 이벤트여부
...
for i in range(1, 4):
    ...
    for ym in baseym:
        ...
        for j in range(len(sex_age_per_index)) :
            ...
            df_sample = df_0_temp.sample(n=count, random_state=1004 ,replace = True)
...
################################
# 4. train 마트 생성 (train_data_set3)
################################
cur.execute(
    f"""
    drop table if exists train_data_set3
    """
)
...
cur.execute(
        f"""
        create table if not exists train_data_set3(
          고객ID                        text
        , 기준년월                      INT
        , history_영화평가              INT
        , history_영화시청완료          INT
...
################################
# 5. test 마트 생성 (test_data_set3)
################################
cur.execute(
    f"""
    drop table if exists test_data_set3
    """
)

cur.execute(
        f"""
        create table if not exists test_data_set3(
          고객ID                        text
        , 기준년월                      INT
        , history_영화평가              INT
        , history_영화시청완료          INT

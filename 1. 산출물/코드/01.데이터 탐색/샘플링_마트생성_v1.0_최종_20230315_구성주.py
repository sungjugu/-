...
cur.execute(
    f"""
    drop table if exists 타겟마트_sample
    """
)
...
cur.execute(
    f"""
    create table if not exists 타겟마트_sample(
      기준년월
    , 고객ID
    , 장르
    , 이벤트여부
...
################################
# 4. train 마트 생성 (train_data_set)
################################
cur.execute(
    f"""
    drop table if exists train_data_set
    """
)
...
cur.execute(
        f"""
        create table if not exists train_data_set(
          고객ID                        text
        , 기준년월                      INT
        , history_영화평가              INT
        , history_영화시청완료          INT
